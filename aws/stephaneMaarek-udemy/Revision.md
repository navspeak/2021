...
* Lab API Gateway- create IAM role for lambda. This role has permission to upload to cloudwatch
  ...
  * Use Lambda Proxy Integration (check box) - There is no integration response in this case as we can't modify request and response from Lambda. In this case Lambda will have to send request body, hearers etc by itself
  ...
  * Deploy API gateway: New Stage dev, prod etc to get an endpoint. You can Enable Throttling (default 10000 request per sends with burst of 5000 requests) Web Application Firewall WAF setting can also be applied.
 
 * API Gateway Security Mechanism :
      * IAM Permissions - leverages sig v4 (for users in ur IAM users)
      * Lambda Authorizer (uses Lambda to validate token passed in header. Can cache result of Authn). Helps with 3rd party OAuth / SAML type use case, lambda returns IAM policy for user
      * Cognito User Pools - No custom impl needed. However only authn not authz. Use headers like Authorization
 ...
 * https://tutorialsdojo.com/amazon-api-gateway/?src=udemy
 ...
 # IAM (pg465)
 * STS - Assume Role - Global Service - Programatic only. returns AccessID | SecretAccessKey | SessionToken | Expiration
 * Federation in AWS:
 ...
 * __Resource Access Manager (RAM)__
  ...
  * AWS SSO
    * Integrated with AWS Organizations. Support SAML 2.0, integration with on-prem AD. Centralized audit with CloudTrail
 ...
 * Cognito: (https://www.youtube.com/watch?v=g2s_hJRI70U)
  ...
  * KMS
    ...
    * 3 Types of CMK - AWS Managed Service Default (free) | User Keys created in KMS $1/ months | User Keys imported $1 /month => + pay for API calls to KMS
    * CMK used to encrypt data can never be retrieved. CMK is rotated for extra security
    * KMS can only encrypt 4 KB of data per call ( > 4 KB => use envelope encryption)
    * Key Policies - default vs custom (with IAM policies)  - read
...
```
* Key rotation fort Customer Managed CMK (not AWS managed CMK) - happens every 1 year. Prev key kept active for old data
* Manual Key rotation - 90, 180 days etc. Keep same alias. CMK ID is different
* __SSM Parameter Store__: Store secrets - encrypt using KMS. Read PDF - pg 515. Std & Advanced (option for Parameter Policy)
...
* __AWS Secrets Manager__: Newer, capability to force rotation of secrets every X days using a Lambda.  Mostly meant for RDS/Redshift/API Key integration
* __Cloud HSM__ => Gives hardware, you manage ur encryption keys entirely. Multi AZ. SSE-C is good option. AWS can't recover the keys
* __Shield - DDoS protection__ : Std (layer 3/layer 4 SYN/UDP flood attacks) | Advanced - waives price dure to DDoS usage spikes
    * Protect Route 53. Also CloudFront, ALB (Which can also have WAF)
* __Web Application Firewall (WAF)__ :
    * Layer 7 exploits. Can be deployed on (1) ALB (2) API Gateway (regional resources) or (3) CloudFront Distributions.
    * Define Web ACL (ules for IP addresses, headers, URI strings etc). XSS, sql injection. Geo match, size constraints
      * Managed Rules or your own
    * Rate based rules - for DDoS protection
    * __AWS Firewall Manager__: manages rules (shield policy, WAF, security group policy) in all accounts of an AWS org
* __Guard Duty__ : ML to detect threat. Crypto Currency attack. Analyze VPC flow Logs, Cloud Trail Logs, DNS logs
* __Inspector__: Automated security assressments from EC2 instances. Known OS vulnerbaility, unintended n/w accessibility. Inspector Agent installled on OS of EC2
 ...
 * Lab: VPC - Description, CIDR Blocks (add more - upto 5), Flow Logs tabs. Comes with __Main Route table & main Network ACL (NACL)__
    * Create Public (usually smaler, put LB) and Private Subnet. 1 Subnet <=> 1 AZ. In Public subnets check "Enable auto-assign public ipv4"
    * __Internet Gateway__ : Created separately from VPC, highly available. Needed  to edit __Route Table__ for internet (needed for ssh or RDP too). Attached to 1 VPC only. Also a NAT for instances that have apiblic IPV4
 ...
 * NAT Gateways: Can't be used by insatce in the subnet where it's created. 5GBps bandwidth that scales automatically to 45 Gbps. No security group to configure
...
* __VPC Endpoints__ : Refer page 561  
    * Is tied to a region. 
 ...
 * __Flow Logs__: page 563 => go to S3 or CloudWatch Logs. Lab: Go to VPC > Flow log Tab > Destination: in cloudwatch create a log group > IAM role: flowlog Role.
 Destination can also be S3 also. In cloudwatch see that there are so many IPs from everywhere in the world
 ...
 * __Site-to-Site VPN__: (pg 568)
    ...
    * 2 IPSecChannel
 * __Direct Connect (DX)__ : One month or so. Not fast to establish. Data is not encrypted though private. Use AWS Direct Connect + VPN provides an IPSec encrypted private connection
    * Customer N/w
  ...
  * __AWS PrivateLink or VPC Endpoint Services__
   ...
   * __Transit Gateway__: hub and spoke star model. Many VPCs thru this GW transitively. IP multicast. ECMP - throughput
               - TGW has its own Route table. VPCS can be in different acc. TGW and VPC in same region (?? no maybe)
               - 5000 attachments (VPC peering only 125)
# Networking extras
- AWS Site-to-Site VPN - creates private connection using IP Sec & TLS.  Still uses public internet (DX does not use)
# Disaster Recovery 
...
```
---------Faster RTO-----?
Back & Restore | Pilot Light | Warm Standby | hot/Multisite
<---AWS Multi Region --->
```
* __Data Migration Service (DMS)__: Supports CDC (change data capture for cont. replication). Schema Conversion Tool (SCT) if engines not same
* On-prem strategies: page 599 - Download Amazon AMI, VM export/import, AWS Application Disovery, AWS DMS, AWS Server Migration
...
- AWS Fargate - serverless. Access tasks using ENI (so have IPs avaible for the ENIs)
  ...
  * ALB can integrate to services in ECS cluster:
    * __Dynamic Port mapping__ to find the right port on your EC2 instances. Must allow SG of EC2 instance to allow any port from ALB
  * Loadbalancing for Fargate: There is ENI for each task. SG: allow ENI's SG the task port from the ALB
  ...
  * ECS Scaling: Service CPU Usage:
    * CloudWatch Metric (ECS Service CPU usage) > triggers cloudWatch Alarm > scales ECS tasks or Scale use __ECS Capacity Provider__ to add more EC2 instance
